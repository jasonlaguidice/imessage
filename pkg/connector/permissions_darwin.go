...
func requestContactAccess(log zerolog.Logger) {
	cs := mac.NewContactStore()
	err := cs.RequestContactAccess()
	if err != nil {
		log.Warn().Err(err).Msg("Failed to get contact access")
	} else if cs.HasContactAccess {
		log.Info().Msg("Contact access granted")
	} else {
		log.Warn().Msg("Contact access denied")
	}
}
